...
### หมายเหตุเรื่อง mixin

- `HandledScreenMixin` inject เข้า `HandledScreen.init()` ที่ `@At("TAIL")` — ทำงานกับ**ทุก** container GUI ที่เปิด (chest, generic_9x6, custom menu) ไม่ใช่แค่ /worth ดังนั้นการกรองด้วย `guiTitle` ใน `EventManager` จึงสำคัญมาก
- ถ้าอยากดีบัก title จริงที่เจอ ให้เปิด `"debug": true` ใน config.json แล้วดู log บรรทัด `Ignoring screen with title "..."`
- ถ้ามันไม่ apply เลย (mod โหลดไม่ error แต่ event ไม่ยิง) เช็คว่า `fabric.mod.json` มี `"mixins": ["price_sync.mixins.json"]`
- **`compatibilityLevel: JAVA_21`** ใน `price_sync.mixins.json` ยืนยันแล้วว่า**ถูกต้อง ไม่ต้องแก้เป็น JAVA_25** — ค่านี้บอกแค่ว่า mixin class ใช้ bytecode feature (records, sealed classes, ฯลฯ) ระดับไหน ไม่ใช่ต้องตรงกับ JDK ที่ compile จริง `HandledScreenMixin` เป็น abstract class ธรรมดา ไม่มี syntax เกิน Java 21 เลย
