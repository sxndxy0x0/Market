...
**ถ้าอยากเก็บให้ครบทั้ง 43 หน้าแบบไม่ต้องกดเอง** ตอนนี้มี **auto-pagination** แล้ว (ดูหัวข้อ "Auto-pagination" ด้านล่าง) เปิดผ่าน `config.autoPaginate: true` — ถ้าไม่เปิด ก็ยังไล่กดดูเองได้ตามปกติ (browse ทั้งหมด mod จะ sync ให้อัตโนมัติทุกหน้าที่คุณผ่านอยู่แล้ว)
...
## Auto-pagination (ทำแล้ว, opt-in — ปิดไว้เป็น default)

ยืนยันจากผู้ใช้แล้วว่า**ไม่ผิดกฎเซิร์ฟเวอร์** และปุ่ม "หน้าถัดไป" คือไอเทมรูปธนู/ลูกศรที่ **slot สุดท้าย (index 53, มุมล่างขวา)** ของ GUI 6 แถว — เพิ่ม `gui/AutoPaginator.java` ให้ mod คลิกปุ่มนี้เองอัตโนมัติ

**วิธีเปิดใช้งาน**: แก้ `config/price-sync/config.json`
```json
"autoPaginate": true,
"nextPageSlot": 53
```
...
**วิธีทำงาน**:
- ใช้ `MultiPlayerGameMode.handleInventoryMouseClick(...)` — เมธอดเดียวกับที่เกมเรียกตอนคุณคลิกเมาส์เองจริงๆ (ไม่ใช่ hack/inject แปลกๆ)
- ทุกรอบ poll (~1 วินาที) เช็คเลขหน้าจาก title (`(N/M)`) ถ้ายังไม่ถึงหน้าสุดท้าย → sync ราคาหน้าปัจจุบันก่อน แล้วค่อยคลิกเปลี่ยนหน้า
- คลิกแค่ 1 ครั้งต่อหน้า (กันคลิกรัวถ้า server ตอบช้า) รอจน title เปลี่ยนเลขหน้าค่อยคลิกต่อ
- หยุดอัตโนมัติเมื่อถึงหน้าสุดท้าย (`current >= total`)
- มี safety cap ที่ **300 คลิกต่อรอบเปิด GUI** กันเผื่อ parse เลขหน้าพลาดจนคลิกไม่หยุด (เช่น title format เปลี่ยนกะทันหัน)
- `resetSession()` ถูกเรียกทุกครั้งที่เปิด GUI ใหม่ที่ตรงชื่อ ทำให้เริ่มนับหน้าใหม่ทุกรอบ

**ยังไม่รองรับ**: ไล่หมวดหมู่ (คอลัมน์ซ้ายมือ) ให้อัตโนมัติด้วย — ตอนนี้แค่ไล่หน้าในหมวดที่เปิดอยู่ ถ้าอยากได้ครบทุกหมวด ต้องกดเปลี่ยนหมวดหมู่เองแล้วปล่อยให้ auto-paginate ไล่หน้าให้ในแต่ละหมวด
