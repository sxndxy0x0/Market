...
public class ConfigManager {
    ...
    public void validate() {
        ...
        if (config.nextPageSlot < 0) {
            Logger.warn("nextPageSlot can't be negative (got " + config.nextPageSlot
                    + "), falling back to " + defaults.nextPageSlot + ".");
            config.nextPageSlot = defaults.nextPageSlot;
        }
    ...
    public static class ModConfig {
        ...
        // Auto-pagination: OFF by default (opt-in). When true, the mod clicks
        // the "next page" slot itself while the /worth GUI is open, syncing
        // each page along the way, until it reaches the last page. Confirm
        // this doesn't violate your server's rules before enabling — it IS a
        // form of automated clicking, even though it's just browsing a menu.
        public boolean autoPaginate = false;
        // Slot index (0-based) of the "next page" arrow button. Confirmed on
        // SIAM SMP+ as the last slot in a 6-row (54-slot) /worth GUI — i.e.
        // bottom-right corner. Adjust if your server's button is elsewhere.
