...
import com.example.pricesync.gui.AutoPaginator;
 ...
 *  1. Fabric API's ScreenEvents.AFTER_INIT fires once when the GUI first opens
 *     (see registerAll below). We used to do this via a hand-written mixin, but
 *     switched to Fabric's own stable ScreenEvents API after repeatedly guessing
 *     wrong about Minecraft's internal current-screen field/method across 26.2 —
 *     ScreenEvents doesn't depend on those internals and is actively maintained
 *     by Fabric alongside each MC version.
...
public class EventManager {
    ...
    private final AutoPaginator autoPaginator;
    ...
    public EventManager(
        ...
        this.autoPaginator = new AutoPaginator(configManager);
    }
    ...
    public void registerAll() {
        ...
        // Fires once whenever ANY container GUI opens (Fabric API's own event,
        // not our own mixin). We only care about it if the title starts with
        // the configured /worth GUI title. NOTE: this server's real title is
        // "WORTH (1/43)" — it has a page number suffix that changes per page,
        // so this MUST be a prefix match, not an exact one, or every page
        // after the first gets ignored.
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (currentTitle != null && titleMatches(currentTitle)) {
                safeRunSyncPipeline();
                try {
                    autoPaginator.maybeAdvance(currentTitle);
                } catch (Exception e) {
                    Logger.error("Auto-paginate failed (swallowed to avoid crashing the game)", e);
                }
            }
        });
    }
    ...
    /**
     * Maps Unicode "small caps" stylized letters (e.g. "ᴡᴏʀᴛʜ") to plain
     * ASCII lowercase letters. Many servers style GUI titles this way for
     * looks — confirmed on a real server where the /worth title renders as
     * "ᴡᴏʀᴛʜ (1/43)", which is NOT the same characters as ASCII "WORTH" and
     * would never match a plain case-insensitive comparison. This lets
     * `guiTitle` in config.json stay a normal, easy-to-type "WORTH" instead
     * of requiring the player to paste special Unicode characters.
     */
    private static final java.util.Map<Character, Character> SMALL_CAPS_TO_ASCII = java.util.Map.ofEntries(
            java.util.Map.entry('ᴀ', 'a'), java.util.Map.entry('ʙ', 'b'), java.util.Map.entry('ᴄ', 'c'),
            java.util.Map.entry('ᴅ', 'd'), java.util.Map.entry('ᴇ', 'e'), java.util.Map.entry('ꜰ', 'f'),
            java.util.Map.entry('ɢ', 'g'), java.util.Map.entry('ʜ', 'h'), java.util.Map.entry('ɪ', 'i'),
            java.util.Map.entry('ᴊ', 'j'), java.util.Map.entry('ᴋ', 'k'), java.util.Map.entry('ʟ', 'l'),
            java.util.Map.entry('ᴍ', 'm'), java.util.Map.entry('ɴ', 'n'), java.util.Map.entry('ᴏ', 'o'),
            java.util.Map.entry('ᴘ', 'p'), java.util.Map.entry('ʀ', 'r'), java.util.Map.entry('ꜱ', 's'),
            java.util.Map.entry('ᴛ', 't'), java.util.Map.entry('ᴜ', 'u'), java.util.Map.entry('ᴠ', 'v'),
            java.util.Map.entry('ᴡ', 'w'), java.util.Map.entry('ʏ', 'y'), java.util.Map.entry('ᴢ', 'z')
    );

    private static String normalizeTitle(String s) {
        StringBuilder sb = new StringBuilder(s.length());
        for (char c : s.toCharArray()) {
            sb.append(SMALL_CAPS_TO_ASCII.getOrDefault(c, c));
        }
    ...
    private boolean titleMatches(String title) {
        ...
        return normalizeTitle(title).startsWith(normalizeTitle(expected));
    }
